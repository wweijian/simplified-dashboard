...
func renderLayout(model Model) string {
	...
	if model.mode == ModeAddFinanceTransaction {
		return overlayCentered(layout, model.addFinanceForm.View(model.width), model.width, model.height)
	}
	if model.mode == ModeEditFinanceTransaction {
		return overlayCentered(layout, model.addFinanceForm.ViewWithTitle(model.width, "Edit transaction"), model.width, model.height)
	}
